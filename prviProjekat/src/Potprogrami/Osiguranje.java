...
public class Osiguranje {
	...
	public static void main(String[] args) {
		...
		do {
			...
			switch(izbor) {
			case '1': unosPrihoda(evidOsig);
				break;
			case '2': povecajPrihod(evidOsig);
				break;
			case '3': sortiraniPrikazKategorijaZaDan(evidOsig);
				break;
			case '4': sortiraniPrikazDanaZaKategoriju(evidOsig);
				break;
			case '5': danSaNajmanjimPrihodom(evidOsig);
				break;
			case '6': prihodPoKategorijama(evidOsig);
				break;
			case '7': mesecniPrihod(evidOsig);
				break;
			case '8': izlistajMatricu(evidOsig, "Dan", "Kategorija");
			}
	...
	static void unosPrihoda(double[][] eo) {
		...
		do {
			dan = preuzmiInt("Unesite dan za koji unosite prihod", 
	...
	static void povecajPrihod(double[][] eo) {
		...
		do {
			dan = preuzmiInt("Unesite dan za koji povećavate prihod", 
					         "Dan mora biti u opsegu 1-"+DANI, 
					         1, DANI);
			...
			/*for (int i = 0; i < KATEGORIJE-1; i++) 
				for (int j = i+1; j < KATEGORIJE; j++) {
					if(sortKategorije[i] < sortKategorije[j]) {
						double tmpD = sortKategorije[i];
						sortKategorije[i] = sortKategorije[j];
						sortKategorije[j] = tmpD;
